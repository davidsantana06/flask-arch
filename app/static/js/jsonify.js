function createJsonResponse(message, category, animationIn, animationOut) {
    ...
    const createChildDiv = () => {
        ...
        childDiv.classList.add("json-response", "alert", `alert-${category}`, "d-flex", "align-items-center", "justify-content-center", "border-0", "px-4", "rounded-pill", "shadow-sm", "animate__animated", animationIn);
    ...
    const removePreviousResponse = () => {
        const previousResponse = document.querySelector(".json-response");
...
function handleResponseData(data) {
    if (data.redirect) {
        ...
        const jsonResponse = createJsonResponse(data.message, data.category, "animate__bounceIn", "animate__fadeOut");
        document.querySelector("script").insertAdjacentElement("beforebegin", jsonResponse);
    }
}
